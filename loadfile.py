...
'''
def views_country(json_data, document_uuid): 
    # Filter data for the specified document UUID
    group_uuid = [entry for entry in json_data if entry.get("subject_doc_id") == document_uuid]
...
def group_country(countries):
    ...
    country_continents = [continents.get(map_country.get(country, "Unknown"), "Unknown") for country in countries] #Iterates through each country in countries list and gets corresponding continent from the map_country dictionary
    continent_count = Counter(country_continents)  # Count occurrence of each continent
    print(continent_count.keys())
...
def readfile():
    ...
    try: 
        with open(file_path) as f:   # Read data from the file
            ...
            result = views_country(json_data, document_uuid)
            print(result)
